...
extract_openwrt() {
    ...
    if ! mount -o compress=zstd:15 ${loop_new}p2 ${tag_rootfs}; then
...
EOF
    ...
    rm -rf ${tag_bootfs}/extlinux ${tag_bootfs}/u-boot-*.bin
    cp -af "${configfiles_path}/patches/meson-g12a-s905l3a-cm311.dtb" ${tag_bootfs}/dtb/amlogic
    cd ${tag_rootfs}/etc && rm -f config/amlogic model_database.txt flippy-openwrt-release docker/daemon.json && cd -
    rm -rf ${tag_rootfs}/usr/sbin/openwrt-* ${tag_rootfs}/lib/u-boot ${tag_rootfs}/lib/firmware/rtlbt
    ln -sf ../init.d/btrtl ${tag_rootfs}/etc/rc.d/S61btrtl
